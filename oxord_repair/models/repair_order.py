...
class RepairOrder(models.Model):
    ...
    technician_line_ids = fields.One2many(
        'repair.technician.line', 'repair_order_id'
    )

    current_technician_id = fields.Many2one(
        'res.users', string='Technician/Engr. Assigned'
    )

    readonly_technician_line = fields.Boolean(
        string="Readonly Technician Line",
        compute="_compute_readonly_technician_line",
        store=True
    )


    # Draft Signature (Customer acknowledgment)
    draft_signature_request_id = fields.Many2one(
        'sign.request',
        string="Draft Signature Request",
        tracking=True
    )
    
    
    receiving_signature_state = fields.Selection([
        ('not_requested', 'Not Requested'),
        ('requested', 'Requested'),
        ('signed', 'Signed'),
    ], default='not_requested', tracking=True, store=True)
    
    # release_signature_state = fields.Selection([
    #     ('not_requested', 'Not Requested'),
    #     ('requested', 'Requested'),
    #     ('signed', 'Signed'),
    # ], default='not_requested', tracking=True, store=True)
    ...
    # def action_request_signature_release(self):
    #     self.ensure_one()
    
    #     self.release_signature_state = 'requested'
    ...
    def action_request_signature_draft(self):
        """Open the sign request wizard and mark state as requested"""
        self.ensure_one()
        self.receiving_signature_state = 'requested'  # mark as requested
    ...
    @api.model
    def _update_receiving_signature_state(self, sign_request):
        """
        Called after a sign.request is signed
        """
        # sign_request.res_model and res_id exist in the wizard context, not on sign.request itself
        for item in sign_request.item_ids:
            if item.res_model == 'repair.order' and item.res_id:
                repair = self.browse(item.res_id)
    ...
    def _onchange_unit_type_for_brand(self):
        ...
        for rec in self:
            ...
            domain_brand = []
            if rec.unit_type_id:
                domain_brand = [('unit_type_ids.unit_type_id', '=', rec.unit_type_id.id)]
    
            return {'domain': {'brand_id': domain_brand}}
    

    
    @api.depends('state')
    def _compute_readonly_technician_line(self):
        for rec in self:
            # Technicians are editable only in draft or under_repair
            rec.readonly_technician_line = rec.state in ('done', 'released', 'cancel')


    def change_technician(self, new_technician_id):
        """Change technician during repair."""
        for order in self:
            ...
            # End any active lines for other technicians
            active_lines = order.technician_line_ids.filtered(
                lambda l: not l.end_date and l.technician_id.id != new_technician_id
            )
            active_lines.write({'end_date': now_dt})
    
            # Create new line if not already active
            existing_line = order.technician_line_ids.filtered(
                lambda l: l.technician_id.id == new_technician_id and not l.end_date
            )
            if not existing_line:
                order.technician_line_ids.create({
    ...
    def action_create_payment_sale_order(self):
        ...
        # Determine labor price based on brand and unit type
        labor_price = 350.0  # Default if nothing matches
        if self.brand_id and self.unit_type_id:
            unit_type_line = self.brand_id.unit_type_ids.filtered(lambda u: u.unit_type_id == self.unit_type_id)
            if unit_type_line:
                labor_price = unit_type_line.labor_amount or labor_price
    ...
    def action_start_repair(self):
        for order in self:
            if not order.technician_line_ids:
                raise UserError(_("Please assign at least one Technician before starting repair."))
    
            now = fields.Datetime.now()
    
            # Start repair
            order.state = 'under_repair'
            order.repair_start_date = now
    
            # Start the first technician line that has no start_date
            first_line = order.technician_line_ids.filtered(lambda l: not l.start_date)[:1]
            if first_line:
                first_line.start_date = now
    ...
    def action_done(self):
        for rec in self:
            ...
            # --- End all active technician lines ---
            active_lines = rec.technician_line_ids.filtered(lambda l: not l.end_date)
            for line in active_lines:
                # Set end date to now
                line.end_date = now
                # Force aging to recompute immediately and freeze
                line.aging_seconds = int((line.end_date - line.start_date).total_seconds())
                line.aging_days = line.aging_seconds // 86400
                line.aging_hours = (line.aging_seconds % 86400) // 3600
                line.aging_minutes = (line.aging_seconds % 3600) // 60
    
                # Format aging nicely (only show relevant units)
                secs = line.aging_seconds
                parts = []
                if secs >= 86400:
                    parts.append(f"{line.aging_days}d")
                if secs >= 3600:
                    parts.append(f"{line.aging_hours}h")
                if secs >= 60:
                    parts.append(f"{line.aging_minutes}m")
                if secs < 60:
                    parts.append(f"{secs}s")
                line.aging_formatted = " ".join(parts)
    
            # --- Mark repair as done ---
            rec.state = 'done'
            rec.message_post(body=f"Repair marked as <b>Done</b> by {self.env.user.name}.")
    
            # --- Create invoice in draft automatically ---
    ...
    def write(self, vals):
        """Track changes, handle void warranty, and auto-manage technician lines."""
        for rec in self:
            changes = []
...
# ----------------------------------------
# ATTACHMENT LISTENER (PUT THIS BELOW)
# ----------------------------------------
class IrAttachment(models.Model):
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            if rec.res_model == 'repair.order' and rec.res_id:
                repair = self.env['repair.order'].browse(rec.res_id)

                if repair.receiving_signature_state == 'requested':
                    repair.receiving_signature_state = 'signed'

                    repair.message_post(
                        body="✅ Receiving document has been signed.",
