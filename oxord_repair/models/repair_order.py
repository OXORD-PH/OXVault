...
class RepairOrder(models.Model):
    ...
    def action_rereturn(self):
        ...
        # Ensure original has a unit type
        if not self.unit_type_id:
            # Optionally, auto-fill a default unit type based on department
            default_unit = self.env['repair.unit.type'].search(
                [('department_id', '=', self.department_id.id)], limit=1
            )
            if not default_unit:
                raise UserError("Cannot create Re-Return: Original Repair has no Unit Type and no default found for this department.")
            unit_type = default_unit
        else:
            unit_type = self.unit_type_id
    
        # Create new repair order
        new_order = self.env['repair.order'].create({
    ...
    def action_create_invoice(self):
        ...
        self.env['account.move.line'].create({
            ...
            'price_unit': 0,
            'name': "=== PROBLEMS ===",
            'display_type': 'line_section',  # <-- Odoo 17 section header
        })
    
        # 2️⃣ Add Problem Lines
        ...
        # 3️⃣ Add HEADER: OTHER (only if labor exists)
        if self.labor_amount and self.labor_amount > 0:
            self.env['account.move.line'].create({
                ...
                'price_unit': 0,
                'name': "=== OTHER ===",
                'display_type': 'line_section',  # <-- header section
            })
    
            # 4️⃣ Add Labor Line (NO PRODUCT)
            self.env['account.move.line'].create({
                'move_id': invoice.id,
                'product_id': False,
                'quantity': 1,
