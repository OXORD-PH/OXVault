...
class RepairOrder(models.Model):
    ...
    def action_print_claim_form(self):
        return self.env.ref('oxord_repair.action_report_repair_order').report_action(self)
    ...
    def action_create_quotation(self):
        ...
        if not self.problem_ids:
            raise UserError(_("Please add at least one problem before creating a quotation."))
        ...
        for problem in self.problem_ids:
            if not problem.product_id:
                # Automatically create a service product if missing
                product = self.env['product.product'].create({
                    'name': problem.name,
                    'type': 'service',
                    'list_price': problem.estimated_cost or 0.0,
                    'sale_ok': True,
                })
                problem.product_id = product
            else:
                product = problem.product_id

            description = f"{problem.department_id.name if problem.department_id else 'No Dept'} - {problem.name}"
            if self.unit_type_id:
                description += f" | Unit: {self.unit_type_id.name}"
            if self.model_id:
                description += f" | Model: {self.model_id.name}"
            if self.serial:
        ...
        # Add labor amount as a separate line if > 0
        ...
        sale_order = self.env['sale.order'].create({
            ...
            'origin': self.name,
            'repair_order_id': self.id,  # link back to the repair order
        })

        self.quotation_id = sale_order.id

        # Return action to open the quotation form but after confirm it returns to the repair order
        return {
    ...
    def action_create_invoice(self):
        for order in self:
            if order.invoice_ids:
                raise UserError("Invoice already created for this repair order.")

            # Get the quotation (sale.order) linked to this repair
            sale_order = self.env['sale.order'].search([('repair_order_id', '=', order.id)], limit=1)
            if not sale_order:
                raise UserError("No quotation found for this repair order.")

            # Create invoice from the sale.order lines
            invoice_vals = {
    ...
    def action_view_quotation(self):
        """Open the linked quotation (sale.order) in form view."""
        self.ensure_one()
        if not self.quotation_id:
            # Simply return an info popup instead of hiding button
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': 'Info',
                    'message': 'No quotation has been created yet.',
                    'type': 'info',
                    'sticky': False,
                }
            }
    ...
    def action_start_repair(self):
        ...
        for rec in self:
            ...
            if not rec.quotation_id:
                raise UserError("You must create a quotation before starting the repair.")
    ...
    def action_done(self):
        for rec in self:
            ...
            if rec.current_tech_start and not rec.current_tech_end:
                ...
                # Handle edge case: no current tech was ever started
    ...
    def create(self, vals):
        # Normalize: make vals always a list
        is_single = isinstance(vals, dict)
        vals_list = [vals] if is_single else vals
    
        for item in vals_list:
            company_id = item.get("company_id") or self.env.company.id
            company = self.env["res.company"].browse(company_id)
    
            service_type = item.get("service_type", "carry_in")
    
            # Company-based sequence selection
            if company.name == "OXORD La Hacienda":
                seq_code = "repair.order.lahacienda"
    
            elif company.name == "OXORD Robinsons":
                seq_code = "repair.order.robinsons"
    
            elif company.name == "OXORD HQ":
                seq_code = "repair.order.hq"
                service_type = "onsite"  # force JO
    
            else:
                seq_code = "repair.order.lahacienda"  # fallback
    
            # Generate sequence
            next_number = self.env["ir.sequence"].next_by_code(seq_code) or "/"
    
            # Replace RO → JO if onsite
            if service_type == "onsite":
                next_number = next_number.replace("/RO/", "/JO/")
    
            # Assign
            item["name"] = next_number
    
        # Pass the correct type back to super
        return super().create(vals_list if not is_single else vals_list[0])
