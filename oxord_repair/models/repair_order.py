...
class RepairOrder(models.Model):
    ...
    def change_technician(self, new_technician_id):
        ...
        for order in self:
            # End the previous technician's aging
            if order.current_technician_id:
                line = order.technician_line_ids.filtered(
                    lambda l: l.technician_id == order.current_technician_id and not l.end_date
                )
                line.end_date = now_dt
    
            # Assign new technician
            order.current_technician_id = new_technician_id
    
            # Start new technician line
            line = order.technician_line_ids.filtered(
                lambda l: l.technician_id == new_technician_id and not l.start_date
            )
            if not line:
    ...
    def action_start_repair(self, technician_id=None):
        """Start repair: set state and assign the first technician via technician_id."""
        for order in self:
            # Use the first assigned technician if no technician_id passed
            technician_id = technician_id or (
                order.technician_line_ids and order.technician_line_ids[0].technician_id.id
            )
            if not technician_id:
                raise UserError(_("Please assign a Technician before starting repair."))
    
            now_dt = fields.Datetime.now()
    
            # Start repair
            order.state = 'under_repair'
            order.repair_start_date = now_dt
    
            # No need to create a technician line here — it's already created when you assign the technician
    
            # Set current technician tracking
            order.current_technician_id = technician_id
            order.current_tech_start = now_dt
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if 'current_technician_id' in vals:
                ...
                # End previous technician line(s) if different
                previous_lines = rec.technician_line_ids.filtered(
                    lambda l: l.end_date is False and l.technician_id != new_tech_id
                )
                previous_lines.write({'end_date': now_dt})
    
                # Check if new technician already has a line without start_date
                line = rec.technician_line_ids.filtered(
                    lambda l: l.technician_id.id == new_tech_id and not l.start_date
                )
                if not line:
                    # Create new line for the new technician
                    self.env['repair.technician.line'].create({
