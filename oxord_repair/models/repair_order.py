...
class RepairOrder(models.Model):
    ...
    def action_create_payment_invoice(self):
        ...
        # Partner receivable account check
        if not self.partner_id.property_account_receivable_id:
            raise UserError(
                f"Partner {self.partner_id.name} does not have a receivable account defined."
            )

        # Default income account for non-product lines
        income_account = self.env['account.account'].search(
            [('account_type', '=', 'income')],
            limit=1
        )
        if not income_account:
            raise UserError("No income account found. Please configure an income account.")
        ...
        invoice.write({'repair_order_id': self.id})
        ...
        # DO NOT POST — user posts & pays manually
        return {
