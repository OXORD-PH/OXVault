...
class RepairOrder(models.Model):
    ...
    def action_create_payment_sale_order(self):
        ...
        if self.invoice_count > 0:
            raise UserError(_("This repair order already has an invoice."))

        # Get the POS session for the current user
        pos_session = self.env['pos.session'].search([
            ('user_id', '=', self.env.user.id),
            ('state', '=', 'opened')
        ], limit=1)
        if not pos_session:
            raise UserError(_("No open POS session found for the current user."))
        ...
        for problem in self.problem_ids:
            if not problem.product_id:
                raise UserError(_("The problem '%s' does not have a related product set." % problem.name))

            price = float(problem.estimated_cost) if problem.estimated_cost else 0.0
            order_lines.append((0, 0, {
                'product_id': problem.product_id.id,
                'product_uom_qty': 1.0,
                'price_unit': price,
                'name': problem.name or '',
            }))

        # Add parts
        for part in self.repair_line_ids:
            if not part.product_id:
                raise UserError(_("A part in this repair order does not have a product set."))

            price = float(part.price_unit) if part.price_unit else 0.0
            quantity = float(part.product_uom_qty) if part.product_uom_qty else 1.0
            order_lines.append((0, 0, {
        ...
        # Add technician labor
        for labor in self.technician_ids:
            if not labor.product_id:
                raise UserError(_("A technician labor entry does not have a product set."))

            price = float(labor.price) if labor.price else 0.0
            order_lines.append((0, 0, {
                'product_id': labor.product_id.id,
                'product_uom_qty': 1.0,
                'price_unit': price,
                'name': labor.name or labor.product_id.name,
            }))

        if not order_lines:
            raise UserError(_("No lines available to create a POS order."))

        # Create POS order
        pos_order = pos_order_obj.create({
        ...
        # Confirm POS order in session
        pos_order.action_pos_order_paid()

        # Link back the POS order to repair
    ...
    def _compute_can_release(self):
        for rec in self:
            rec.can_release = bool(
                rec.state == 'done'
                and rec.invoice_ids
                and all(
                    inv.state == 'posted' and inv.payment_state == 'paid'
    ...
    def action_release(self):
        """Release repair order ONLY when invoice is fully paid"""
        for rec in self:
            ...
            unpaid = rec.invoice_ids.filtered(
                lambda inv: inv.state != 'posted' or inv.payment_state != 'paid'
            )
            if unpaid:
                raise UserError(
                    "Cannot release this repair order.\n\n"
                    "The customer invoice is not fully paid."
                )
    ...
    def create(self, vals_list):
        sequence_map = {
            ...
            ('la_hacienda', 'onsite'): 'repair.st4.jo',
            ('robinsons', 'carry_in'): 'repair.st3.wo',
            ('robinsons', 'onsite'): 'repair.st3.jo',
            ('hq', 'carry_in'): 'repair.hq.wo',
            ('hq', 'onsite'): 'repair.hq.jo',
        }
