...
class RepairOrder(models.Model):
    ...
    is_pos_paid = fields.Boolean(
        string="Paid in POS",
        compute="_compute_is_pos_paid",
        store=True
    )
    ...
    def _compute_is_pos_paid(self):
        for rec in self:
            if not rec.sale_order_id:
                rec.is_pos_paid = False
                continue
    
            pos_orders = self.env['pos.order'].search([
                ('state', '=', 'paid'),
                ('lines.sale_order_line_id.order_id', '=', rec.sale_order_id.id),
            ], limit=1)
    
            rec.is_pos_paid = bool(pos_orders)
    ...
    def action_release(self):
        """Release repair order ONLY when POS payment is completed"""
        for rec in self:
            ...
            if not rec.sale_order_id:
                raise UserError("Cannot release this repair order: no Sales Order linked.")
    
            if not rec.is_pos_paid:
                raise UserError(
                    "Cannot release this repair order.\n\n"
                    "The repair has not been paid in POS."
                )
