...
class ProductBrandUnitType(models.Model):
    ...
    unit_type_id = fields.Many2one('repair.unit.type', string="Unit Type", tracking=True, required=True)
    labor_amount = fields.Float(string="Labor Amount", tracking=True, required=True)
    ...
    def write(self, vals):
        """Override write to log changes in the parent brand chatter without HTML formatting"""
        # Track changes for message
        changes = []
        for field in ['labor_amount', 'service_type', 'unit_type_id']:
            if field in vals:
                for rec in self:
                    old = rec[field]
                    new = vals[field]
                    # If unit_type_id, display the name
                    if field == 'unit_type_id':
                        old = rec.unit_type_id.name if rec.unit_type_id else ''
                        new = self.env['repair.unit.type'].browse(vals['unit_type_id']).name
                    if old != new:
                        # Plain text message
                        changes.append(f"{field.replace('_',' ').title()}: {old} → {new} (Unit: {rec.unit_type_id.name})")
    
        res = super().write(vals)
    
        # Post message to parent brand if there are changes
        if changes:
            for rec in self:
                rec.brand_id.message_post(
                    body="\n".join(changes),
                    subject="Unit Type updated"
                )
        return res
